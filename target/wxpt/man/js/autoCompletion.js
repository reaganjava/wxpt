	...
	function mSift(cVarName,nMax,aa){
		this.oo=cVarName;
		this.Max=nMax;
		this.AA= aa;
	} 
	...
	mSift.prototype={ 
		...
		Select:function(){ 
			...
			if(_this.ReData.length>0){ 
				_this.Target.value=_this.ReData[_this.SelIndex].replace(/\*/g,'*').replace(/\|/g,'|');//.split(',')[0]; 
				//alert(_this.ReData[_this.SelIndex].replace(/\*/g,'*').replace(/\|/g,'|').split(',')[1]);
				//alert(this.inputId);
				//alert(_this.SelIndex);
				document.getElementById("hiddenId").value=this.AA[_this.SelIndex];
			    //_this.ReData[_this.SelIndex].replace(/\*/g,'*').replace(/\|/g,'|').split(',')[1];
				_this.Clear();
			} 
		...
		Get:function(){ 
			...
			//alert(_this.Target.value.replace(/\*/g,'*'));
			...
			for(var i=0;i<_this.ReData.length;i++){ 
				...
				+_this.ReData[i].replace(cRegEx,function(s){return '<span style="background:#ff9;font-weight:bold;font-style:normal;color:#ee6600;">'+s+'</span>';});+'</li>';
			} 
			if(cResult==''){
				_this.Clear();
			}else{ 
				...
				_this.Target.offsetWidth =_this.TgList.offsetWidth+'px';  
				//把查询出来的内容显示长度由原来的输入框width修改成为文字长度，从而不折行

...
<script type="text/javascript">
	...
	function init() {
		 ...
		 document.updateForm.url.value='${MENUITEM_EDIT_DATEIL.menuItem.url}';
		 document.updateForm.priority.value=${MENUITEM_EDIT_DATEIL.menuItem.priority};
	}
</script>
</head>
<body onload="init()">
	<div class="form-container">
		<form id="updateForm" name="updateForm" method="post"
			action="${rootpath}menuItem/edit.json">
			<input type="hidden" value="${MENUITEM_EDIT_DATEIL.menuItem.menuId}" name="menuItem.menuId">
			<fieldset>
				<legend> 基本信息 </legend>
				<div>
					<label for="title"> 分组名：<em>*</em>
					</label> <input name="menuItem.title" class="easyui-validatebox" type="text"
						required="true" value="${MENUITEM_EDIT_DATEIL.menuItem.title}" readonly="readonly"/>
				</div>
				<div>
				...
				<div>
					<label for="group">菜单分组：<em>*</em></label>
					 	<c:forEach var="group" items="${GROUP_LIST}">
					 		...
					 		<c:forEach var="groupIds" items="${MENUITEM_EDIT_DATEIL.groupIds}">
					 			 <c:if test="${groupIds == group.groupId}">checked</c:if></c:forEach>>${group.name}
